...
//    서버의 응답이 돌아올 때 실행할 내용을 담아둘 목적의 인터페이스
...
//        로그인 요청을 해주는 기능
        ...
        fun postRequestLogin(context: Context, email: String, pw: String, handler: JsonResponseHandler?) {  // handler는 null 여지 허용
...
//            서버에 요청할 모든 정보를 담아주는 request 변수 생성
            val request = Request.Builder()
                .url(urlString)
                .post(formData)
//                .header()  //API에서 헤더를 요구하면 여기서 첨부해야 함.
                .build()

//            완성된 Request를 가지고 실제로 서버로 출발.
//            서버는 Request를 받으면 => Response를 내려줌.
//            그에 대한 처리도 필요함 (Callback 익명 클래스)
...
//        프로젝트에 참가 신청해주는 기능
        ...
        fun postRequestProjectJoin(context: Context, projectId: Int, handler: JsonResponseHandler?) {  // handler는 null 여지 허용
...
//        인증글에 좋아요 누르는 기능
        ...
        fun postRequestLikeProof(context: Context, proofId: Int, handler: JsonResponseHandler?) {  // handler는 null 여지 허용
...
//        댓글에 좋아요 누르는 기능
        ...
        fun postRequestLikeReply(context: Context, replyId: Int, handler: JsonResponseHandler?) {  // handler는 null 여지 허용
...
//        인증글 업로드 기능
        ...
        fun postRequestProof(context: Context, projectId: Int, content: String, handler: JsonResponseHandler?) {  // handler는 null 여지 허용
...
//        알림 목록 확인 처리 기능
        ...
        fun postRequestNotiConfirm(context: Context, notiId: Int, handler: JsonResponseHandler?) {  // handler는 null 여지 허용
...
//            GET / DELETE 방식의 파라미터 첨부는 query 에 달아야 함.
...
//                    가공된 JSON을 로그로 출력 (bodyString을 한번 JSON으로 변환하는 인코딩 거쳐서 다시 String 출력해야 우리가 알아볼 수 있는 언어로 나옴)
